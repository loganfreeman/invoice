...
class HotelService extends BaseService
{
    ...
    public function getDatatable($search)
    {
        ...
        if(!Utils::hasPermission('view_all')){
            $query->where('Hotels.user_id', '=', Auth::user()->id);
        }
    ...
    protected function getDatatableColumns($entityType, $hideClient)
    {
        return [
            ...
            [
                'Hotel_date',
                function ($model) {
                    if(!Hotel::canEditItemByOwner($model->user_id)){
                        return Utils::fromSqlDate($model->Hotel_date);
                    }

                    return link_to("Hotels/{$model->public_id}/edit", Utils::fromSqlDate($model->Hotel_date))->toHtml();
                }
            ],
            [
                'amount',
                function ($model) {
                    ...
                    if ($model->exchange_rate != 1) {
                        ...
                        return Utils::formatMoney($model->amount, $model->Hotel_currency_id) . ' | ' .
    ...
    protected function getDatatableColumnsVendor($entityType, $hideClient)
    {
        return [
            [
                'Hotel_date',
                function ($model) {
                    return Utils::dateToString($model->Hotel_date);
                }
            ],
            [
                'amount',
    ...
    protected function getDatatableActions($entityType)
    {
        return [
            [
                ...
                function ($model) {
                    return URL::to("Hotels/{$model->public_id}/edit") ;
                },
                function ($model) {
